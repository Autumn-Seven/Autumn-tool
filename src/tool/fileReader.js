        ...
        files.forEach(function(file){
            ...
            fs.stat(path + '/' + file, function(err, stat){
                ...
                if(stat.isDirectory()){
                    ...
                    if(Arr[type]){
                        Arr[type].push({index:index,type:type,path:[path+'/'+file]});
                    }else {
                        Arr[type] = [{index:index,type:type,path:[path+'/'+file]}];
                    }
...
setTimeout(function () {
    ...
    Object.keys(Arr).forEach(function (type) {
        ...
        List.forEach(function (item) {
            ...
            let path = item.path;
            console.log(`		"${index}":{index:${index},type:"${type}",path:require("${path}") ,name:""},`);
        })
